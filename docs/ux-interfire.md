...
| Surface | Role |
| --- | --- |
| `interfirectl` | One-shot commands only (`ping`, `status`, rules / prompts / dns / audit) |
...
scripted and one-command checks stay on the CLI, browse/answer flows stay in
the TUI (and later the tray).
